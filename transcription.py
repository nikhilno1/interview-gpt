...
def run_transcription_app(recordings_folder):
    ...
    st.markdown('<small>Click the mic to start/stop the recording. Then click Analyze. <i>(Max 2 mins.)</i></small>', unsafe_allow_html=True)    
   
    # Record Audio tab
    audio_bytes = audio_recorder(energy_threshold=(-1.0, 1.0),  pause_threshold=130.0)
    if audio_bytes:
